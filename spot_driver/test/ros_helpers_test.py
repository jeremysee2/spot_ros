...
from bosdyn.client.frame_helpers import add_edge_to_tree, VISION_FRAME_NAME, BODY_FRAME_NAME, ODOM_FRAME_NAME
...
class TestPopulateTransformStamped(unittest.TestCase):
    def test_populate_transform_stamped(self):
        ...
        transform_stamped = ros_helpers.populateTransformStamped(current_time, "parent_frame", "child_frame", transform)
...
class TestGetEStopStateFromState(unittest.TestCase):
    def test_get_estop_state_from_state(self):
        ...
        self.assertEquals(estop_state_array.estop_states[0].state, robot_state_pb2.EStopState.STATE_ESTOPPED)
        self.assertEquals(estop_state_array.estop_states[0].type, robot_state_pb2.EStopState.TYPE_HARDWARE)
        ...
        self.assertEquals(estop_state_array.estop_states[1].state, robot_state_pb2.EStopState.STATE_NOT_ESTOPPED)
        self.assertEquals(estop_state_array.estop_states[1].type, robot_state_pb2.EStopState.TYPE_SOFTWARE)
    ...
    def test_get_feet_from_state(self):
        ...
        state.foot_state.add(
            ...
            terrain=robot_state_pb2.FootState.TerrainState(
                ...
                ground_contact_normal_rt_frame=geometry_pb2.Vec3(x=16.0, y=17.0, z=18.0),
        ...
        self.assertEquals(foot_state_array.states[0].contact, robot_state_pb2.FootState.CONTACT_MADE)
        ...
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_distance_rt_frame.x, 1.0)
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_distance_rt_frame.y, 2.0)
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_distance_rt_frame.z, 3.0)
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_velocity_rt_frame.x, 4.0)
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_velocity_rt_frame.y, 5.0)
        self.assertEquals(foot_state_array.states[0].terrain.foot_slip_velocity_rt_frame.z, 6.0)
        self.assertEquals(foot_state_array.states[0].terrain.ground_contact_normal_rt_frame.x, 7.0)
        self.assertEquals(foot_state_array.states[0].terrain.ground_contact_normal_rt_frame.y, 8.0)
        self.assertEquals(foot_state_array.states[0].terrain.ground_contact_normal_rt_frame.z, 9.0)
        self.assertEquals(foot_state_array.states[0].terrain.visual_surface_ground_penetration_mean, 0.1)
        self.assertEquals(foot_state_array.states[0].terrain.visual_surface_ground_penetration_std, 0.02)
        ...
        self.assertEquals(foot_state_array.states[1].contact, robot_state_pb2.FootState.CONTACT_LOST)
        ...
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_distance_rt_frame.x, 10.0)
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_distance_rt_frame.y, 11.0)
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_distance_rt_frame.z, 12.0)
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_velocity_rt_frame.x, 13.0)
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_velocity_rt_frame.y, 14.0)
        self.assertEquals(foot_state_array.states[1].terrain.foot_slip_velocity_rt_frame.z, 15.0)
        self.assertEquals(foot_state_array.states[1].terrain.ground_contact_normal_rt_frame.x, 16.0)
        self.assertEquals(foot_state_array.states[1].terrain.ground_contact_normal_rt_frame.y, 17.0)
        self.assertEquals(foot_state_array.states[1].terrain.ground_contact_normal_rt_frame.z, 18.0)
        self.assertEquals(foot_state_array.states[1].terrain.visual_surface_ground_penetration_mean, 0.2)
        self.assertEquals(foot_state_array.states[1].terrain.visual_surface_ground_penetration_std, 0.03)
...
class TestGetOdomFromState(unittest.TestCase):
    def test_get_odom_from_state(self):
        ...
        none_tform_example = geometry_pb2.SE3Pose(position=geometry_pb2.Vec3(x=0, y=0, z=0),
                                                  rotation=geometry_pb2.Quaternion(x=0, y=0, z=0, w=1))
        edges = {}
        edges = add_edge_to_tree(edges, body_tform_example, BODY_FRAME_NAME, ODOM_FRAME_NAME)
        edges = add_edge_to_tree(edges, vision_tform_example, BODY_FRAME_NAME, VISION_FRAME_NAME)
        ...
        self.assertEquals(odometry_msg.pose.pose.position.x, -vision_tform_example.position.x)
        self.assertEquals(odometry_msg.pose.pose.position.y, -vision_tform_example.position.y)
        self.assertEquals(odometry_msg.pose.pose.position.z, -vision_tform_example.position.z)
        self.assertEquals(odometry_msg.pose.pose.orientation.x, vision_tform_example.rotation.x)
        self.assertEquals(odometry_msg.pose.pose.orientation.y, vision_tform_example.rotation.y)
        self.assertEquals(odometry_msg.pose.pose.orientation.z, vision_tform_example.rotation.z)
        self.assertEquals(odometry_msg.pose.pose.orientation.w, vision_tform_example.rotation.w)

        # Test with odom frame transformation
        odometry_msg = ros_helpers.GetOdomFromState(state, spot_wrapper, False)
        self.assertEquals(odometry_msg.pose.pose.position.x, -body_tform_example.position.x)
        self.assertEquals(odometry_msg.pose.pose.position.y, -body_tform_example.position.y)
        self.assertEquals(odometry_msg.pose.pose.position.z, -body_tform_example.position.z)
        self.assertEquals(odometry_msg.pose.pose.orientation.x, body_tform_example.rotation.x)
        self.assertEquals(odometry_msg.pose.pose.orientation.y, body_tform_example.rotation.y)
        self.assertEquals(odometry_msg.pose.pose.orientation.z, body_tform_example.rotation.z)
        self.assertEquals(odometry_msg.pose.pose.orientation.w, body_tform_example.rotation.w)

class TestGetWifiFromState(unittest.TestCase):
    def test_get_wifi_from_state(self):
        state = robot_state_pb2.RobotState()
        spot_wrapper = TestSpotWrapper()
        initial_wifi_state = robot_state_pb2.WiFiState(current_mode=robot_state_pb2.WiFiState.MODE_ACCESS_POINT, essid="test_essid")
        state.comms_states.add(wifi_state=initial_wifi_state)

        wifi_state = ros_helpers.GetWifiFromState(state, spot_wrapper)
        self.assertEquals(wifi_state.current_mode, robot_state_pb2.WiFiState.MODE_ACCESS_POINT)
...
class TestGenerateFeetTF(unittest.TestCase):
    def test_generate_feet_tf(self):
        foot_state_msg = FootStateArray([
            ...
            FootState(foot_position_rt_body=geometry_pb2.Vec3(x=10.0, y=11.0, z=12.0))
        ])
...
class TestGetTFFromState(unittest.TestCase):
    ...
    def test_get_tf_from_state_vision(self):
        ...
        edges = add_edge_to_tree(edges, body_tform_example, BODY_FRAME_NAME, ODOM_FRAME_NAME)
        edges = add_edge_to_tree(edges, vision_tform_example, BODY_FRAME_NAME, VISION_FRAME_NAME)
        edges = add_edge_to_tree(edges, special_tform_example, "special_frame", BODY_FRAME_NAME)
...
class TestGetBatteryStatesFromState(unittest.TestCase):
    ...
    def test_get_battery_states_from_state_one(self):
        ...
        self.assertEquals(battery_states.battery_states[0].temperatures, [25.0, 26.0, 27.0])
        self.assertEquals(battery_states.battery_states[0].status, robot_state_pb2.BatteryState.STATUS_DISCHARGING)
    ...
    def test_get_battery_states_from_state_two(self):
        ...
        self.assertEquals(battery_states.battery_states[1].temperatures, [35.0, 36.0, 37.0])
        self.assertEquals(battery_states.battery_states[1].status, robot_state_pb2.BatteryState.STATUS_CHARGING)

class TestGetPowerStatesFromState(unittest.TestCase):
    def test_get_power_states_from_state_off(self):
        state = robot_state_pb2.RobotState()
        state.power_state.motor_power_state = robot_state_pb2.PowerState.MOTOR_POWER_STATE_OFF
        state.power_state.shore_power_state = robot_state_pb2.PowerState.SHORE_POWER_STATE_ON
        spot_wrapper = TestSpotWrapper()
        power_state_msg = ros_helpers.GetPowerStatesFromState(state, spot_wrapper)
        self.assertEquals(power_state_msg.motor_power_state, robot_state_pb2.PowerState.MOTOR_POWER_STATE_OFF)
        self.assertEquals(power_state_msg.shore_power_state, robot_state_pb2.PowerState.SHORE_POWER_STATE_ON)

    def test_get_power_states_from_state_motor_on(self):
        state = robot_state_pb2.RobotState()
        state.power_state.motor_power_state = robot_state_pb2.PowerState.MOTOR_POWER_STATE_ON
        state.power_state.shore_power_state = robot_state_pb2.PowerState.SHORE_POWER_STATE_OFF
        spot_wrapper = TestSpotWrapper()
        power_state_msg = ros_helpers.GetPowerStatesFromState(state, spot_wrapper)
        self.assertEquals(power_state_msg.motor_power_state, robot_state_pb2.PowerState.MOTOR_POWER_STATE_ON)
        self.assertEquals(power_state_msg.shore_power_state, robot_state_pb2.PowerState.SHORE_POWER_STATE_OFF)
...
class TestGetDockStatesFromState(unittest.TestCase):
    def test_get_dock_states_from_state(self):
        ...
        self.assertEquals(dock_state_msg.status, docking_pb2.DockState.DOCK_STATUS_DOCKED)
        self.assertEquals(dock_state_msg.dock_type, docking_pb2.DOCK_TYPE_SPOT_DOCK)
        self.assertEquals(dock_state_msg.dock_id, 3)
        self.assertEquals(dock_state_msg.power_status, docking_pb2.DockState.LINK_STATUS_CONNECTED)
...
class TestGetBehaviorFaults(unittest.TestCase):
    def test_get_behavior_faults(self):
        ...
        self.assertEquals(behavior_faults[0].cause, robot_state_pb2.BehaviorFault.CAUSE_FALL)
        self.assertEquals(behavior_faults[0].status, robot_state_pb2.BehaviorFault.STATUS_UNCLEARABLE)
        ...
        self.assertEquals(behavior_faults[1].cause, robot_state_pb2.BehaviorFault.CAUSE_LEASE_TIMEOUT)
        self.assertEquals(behavior_faults[1].status, robot_state_pb2.BehaviorFault.STATUS_CLEARABLE)
...
class TestGetBehaviorFaultsFromState(unittest.TestCase):
    def test_get_behvaior_faults_from_state(self):
        ...
        self.assertEquals(behavior_faults.faults[0].cause, robot_state_pb2.BehaviorFault.CAUSE_FALL)
        self.assertEquals(behavior_faults.faults[0].status, robot_state_pb2.BehaviorFault.STATUS_UNCLEARABLE)
        ...
        self.assertEquals(behavior_faults.faults[1].cause, robot_state_pb2.BehaviorFault.CAUSE_LEASE_TIMEOUT)
        self.assertEquals(behavior_faults.faults[1].status, robot_state_pb2.BehaviorFault.STATUS_CLEARABLE)
...
class TestGetSystemFaults(unittest.TestCase):
    def test_get_system_faults(self):
        ...
        system_faults = ros_helpers.GetSystemFaults(system_fault_state.faults, spot_wrapper)
        ...
        self.assertEquals(system_faults[0].severity, robot_state_pb2.SystemFault.SEVERITY_WARN)
        ...
        self.assertEquals(system_faults[1].severity, robot_state_pb2.SystemFault.SEVERITY_CRITICAL)
...
class TestGetSystemFaultsFromState(unittest.TestCase):
    def test_get_system_faults_from_state(self):
        ...
        self.assertEquals(system_faults.faults[0].severity, robot_state_pb2.SystemFault.SEVERITY_WARN)
        ...
        self.assertEquals(system_faults.historical_faults[0].error_message, "error message2")
        self.assertEquals(system_faults.historical_faults[0].attributes, ["wifi", "vision"])
        self.assertEquals(system_faults.historical_faults[0].severity, robot_state_pb2.SystemFault.SEVERITY_CRITICAL)
